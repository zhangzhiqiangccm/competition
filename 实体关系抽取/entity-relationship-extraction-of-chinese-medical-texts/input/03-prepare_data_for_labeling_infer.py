...
def replace_empty_infer_predicate_to_three_possible_values(predicate_score_value):
    label_list = open("./predicate_classification/calssification_labels.txt").read().split("\n")
...
def prepare_data_for_subject_object_labeling_infer(predicate_classifiction_input_file_dir,  predicate_classifiction_infer_file_dir, out_file):
    """
    ...
    """
    text_file = open(os.path.join(predicate_classifiction_input_file_dir, "test.txt"), "r", encoding='utf-8').readlines()

    new_epochs_ckpt_dir = get_latest_model_predict_data_dir(predicate_classifiction_infer_file_dir)
    predicate_predict_file = open(os.path.join(new_epochs_ckpt_dir, "predicate_predict.txt"), "r", encoding='utf-8').readlines()
    predicate_score_value_file = open(os.path.join(new_epochs_ckpt_dir, "predicate_score_value.txt"), "r", encoding='utf-8').readlines()
    ...
    for text,  predicate_predict, predicate_score_value in zip(text_file, predicate_predict_file, predicate_score_value_file):
        count_line += 1
        predicate_list = predicate_predict.strip().replace("\n", "").split(" ")
        ...
        for predicate in predicate_list:
            ...
            output_text_file_write.write(str(count_line)+"\t"+" ".join(list(text.replace(" ","").replace("\n", "").split("\t")[1])) + "\t" + " ".join(list(predicate.replace("|||",""))) + "\n")
...
if __name__=="__main__":
    predicate_classifiction_input_file_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "./predicate_classification/"))
    ...
    out_file = os.path.abspath(os.path.join(os.path.dirname(__file__), "./subject_object_labeling/"))
    prepare_data_for_subject_object_labeling_infer(predicate_classifiction_input_file_dir, predicate_classifiction_infer_file_dir, out_file)
